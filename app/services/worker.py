...
from app.core.logging import new_correlation_id, correlation_id_var
...
async def poll_due_slots(db: AsyncSession | None = None) -> None:
    ...
    session_generator = AsyncSessionLocal() if db is None else None
    session = db if db is not None else session_generator
